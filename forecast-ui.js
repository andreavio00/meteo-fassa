...
(function(){
  ...
  function upgradePreview(){
    ...
    const label=m?`${m[1]} ${rangeLabel(m[2])}`:when;
    preview.className="forecast-preview";
    preview.innerHTML=`<span class="forecast-preview-time">${label}</span><span class="forecast-preview-weather">${parts[1].innerHTML}</span><strong class="forecast-preview-temp">${parts[2].textContent.trim()}</strong><span class="forecast-preview-rain">${parts[3].textContent.trim()}</span>`;
  }
